...
(async () => {
  ...
  const testEmail = `lawyer.${uniqueId}@sloelawtest.com`;
  const firmName = `Apex Legal Group ${uniqueId}`;
  const clientName = `Apex Client ${uniqueId}`;
  const matterTitle = `Acquisition & Asset Purchase #${uniqueId}`;
  ...
  console.log(`Matter Title: ${matterTitle}`);
  ...
  console.log('Waiting for signup submission...');
  ...
  await page.waitForTimeout(1500);
  await page.screenshot({ path: 'live_04_conflict_check_cleared.png' });
